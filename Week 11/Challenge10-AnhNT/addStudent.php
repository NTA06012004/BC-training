...
$sql_max_id = "SELECT MAX(user_id) as max_id FROM users;"; 
$result_max_id = select($sql_max_id);
$max_id = $result_max_id[0]['max_id']; 
...
if (!empty($_POST)) {
    ...
    if ($full_name != '' && $password != '' && $email != '' && $username != '' && $phonenumber != '' && $role != '') {
        ...
        $sql = "insert into users (user_id, full_name, username, password, email, phone_number, role_id) values ('$user_id','$full_name','$username' , '$password', '$email', '$phonenumber','$role')";
        execute($sql);
